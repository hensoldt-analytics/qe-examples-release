 ...
 * --master local[*] \
 * --class org.apache.spark.examples.streaming.NewApiDirectKafkaWordCount \
 * spark2-examples-2.1.0.jar <broker-list> <group-id> <topic-list> <additional-kafka-params>
 *
 ...
 */
object NewApiDirectKafkaWordCount {

  def main(args: Array[String]): Unit = {
    if (args.length < 3) {
      System.err.println(
        "NewApiDirectKafkaWordCount: bootstrap_server_list group_id topic_list [consumer.cfg]")
      System.exit(-1)
    }
